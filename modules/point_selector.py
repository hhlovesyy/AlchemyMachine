...
#             self.point_radius = 3
#             # 手绘模式下通常不让画障碍物，保持纯净，或者允许画但会被轨迹覆盖
#             # 这里我们专注于画轨迹
...
#         with col_input:
#             st.markdown("### 👈 绘制")
#             bg_img = self._generate_grid_background(self.current_world_range, overlay_path=None) # 输入端永远不画生成的线，保持干净
            
#             # 动态设置参数
#             s_width = self.stroke_width if "手绘" in self.work_mode else 2
...
#                 height=self.CANVAS_SIZE,
#                 width=self.CANVAS_SIZE,
#                 drawing_mode=self.canvas_mode, # 动态切换 point/rect/freedraw
#                 point_display_radius=self.point_radius,
...
#                     }
#                 elif "手绘" in self.work_mode and freehand_path_arr is not None:
#                     # 手绘点可能很多，建议采样
#                     traj_data = {
#                         "type": "freehand_curve",
#                         # 简单转为 list
